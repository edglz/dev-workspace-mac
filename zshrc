...
typeset -ga WORKSPACE_TOOLS=(
  ...
  'difft|Modern|diff|Difftastic: structural diff aware of language|difft a.js b.js'
  ...
  'xh|Modern|curl|HTTPie-like client in Rust (~30% faster)|xh GET api.github.com/users/octocat'
  ...
  'ngrok|Cloud|-|HTTP/TCP tunnel to expose localhost|ngrok http 3000'
  'cloudflared|Cloud|ngrok|Cloudflare tunnel (free, no limits)|cloudflared tunnel --url http://localhost:3000'
  ...
  'minikube|K8s|-|Local K8s cluster in VM|minikube start'
  'kind|K8s|minikube|Local K8s in Docker containers (lighter)|kind create cluster'
  ...
  'mas|Misc|-|Mac App Store CLI|mas list'
